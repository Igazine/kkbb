...
* **Zero-bloat Approach**: Written in plain Swift using only core macOS components and libraries. The release version of the app is **smaller than 4 MB**, even the debug build takes up **less than 6 MB** of disk space, while the downloadable release ZIP is only 1MB (yes, it *is* that small). The memory consumption remains well within a **60 MB** limit even under heavy use. The app will automatically clean up orphaned virtual MIDI ports when it is quit.
